...
//! Provides and implements the functionality for an [`Unsigned<N>`] type
...
/// Trait used to implement the trait LUT for [`Unsigned<N>`] backing types
pub const trait HasUnsignedBacker: Sized {
    /// The inner type for a given size of [`Unsigned<N>`]
...
/// Retrieves the inner type for an [`Unsigned<N>`]
