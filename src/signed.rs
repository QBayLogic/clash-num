...
//! Provides and implements the functionality for a [`Signed<N>`] type
...
pub const trait HasSignedBacker<const N: u8>: Sized {
    ...
    /// Checks that the inner value is within allowed bounds
...
/// Retrieves the inner type for an [`Signed<N>`]
...
impl<const N: u8> const HasSignedBacker<N> for SignedLutInner<N, 1> {
    ...
    const MASK: Self::Type = { if N == 8 { !0 } else { !(!0 << N) } };
...
impl<const N: u8> const HasSignedBacker<N> for SignedLutInner<N, 2> {
    ...
    const MIN: Self::Type = 0b1 << (N - 1);
    const MAX: Self::Type = !(!0 << (N - 1));
    const MASK: Self::Type = { if N == 16 { !0 } else { !(!0 << N) } };
