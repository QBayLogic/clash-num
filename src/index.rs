...
//! Provides and implements the functionality for an [`Index<N>`] type
...
/// Retrieves the inner type for an [`Index<N>`]
...
/// Alias function for [`HasIndexBacker::bounds_check`]
...
/// Alias function for [`HasIndexBacker::apply_mask`]
...
/// Alias function for [`HasIndexBacker::apply_mask_to`]
...
#[cfg(debug_assertions)]
/// Performs a bounds check on a given [`Index<N>`] backer, panics on failure
///
/// # Panics
///
/// Panics if `val` exceeds the bounds permitted by [`Index<N>`], or in other words, if
...
/// Performs a bounds check on a given [`Index<N>`] backer, calls [`apply_mask`] if the check fails
pub const fn masked<const N: u128>(val: IndexBackerOf<N>) -> IndexBackerOf<N>
where
    IndexLut<N>: const HasIndexBacker,
{
...
#[cfg(debug_assertions)]
/// Performs an in-place bounds check on a given [`Index<N>`] backer, panics on failure
///
/// # Panics
///
/// Panics if `val` exceeds the bounds permitted by [`Index<N>`], or in other words, if
...
#[cfg(not(debug_assertions))]
/// Performs a bounds check on a given [`Index<N>`] backer, calls [`apply_mask_to`] if the check
/// fails
pub const fn masked_inplace<const N: u128>(val: &mut IndexBackerOf<N>)
where
    IndexLut<N>: const HasIndexBacker,
...
where
    IndexLut<N>: const HasIndexBacker,
{
    /// The minimum value permitted for an [`Index<N>`]
    pub const MIN: Self = Index(<IndexLut<N> as HasIndexBacker>::MIN);
    /// The maximum value permitted for an [`Index<N>`]
    pub const MAX: Self = Index(<IndexLut<N> as HasIndexBacker>::MAX);

    /// Create a new [`Index<N>`]
    ///
    /// Returns `Some(_)` if `val` is within bounds, otherwise returns `None`.
    ...
    /// Returns the inner value of an [`Index<N>`]
...
mod test {
    ...
    #[test]
    #[should_panic]
    fn test_index_size_zero() {
        let _ = index_bit_size(0);
    }
